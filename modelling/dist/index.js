...
const Pring = __importStar(require("pring"));
const firebase_1 = __importDefault(require("firebase"));
require("firebase/firestore");
const config = __importStar(require("./firebase.json"));
...
const tests = async () => {
    ...
    chai_1.expect(followers).to.have.members([users[1], users[2], users[3], users[4]], 'Failed to add users as follower on first user');
    ...
    await userService.toggleFollow(users[0], users[2]);
    followers = (await userService.getFollowers(users[0])).map(item => item.id);
    chai_1.expect(followers).to.have.members([users[1], users[3], users[4]], 'The deleted user is not removed on followings on first user');
    console.log('The deleted user is removed on the followings of first user');
    await userService.toggleFollow(users[5], users[2]);
    followers = (await userService.getFollowers(users[5])).map(item => item.id);
    chai_1.expect(followers).to.have.members([users[1], users[3], users[4]], 'The deleted user is not removed on followings on fifth user');
    ...
    chai_1.expect(favorites).to.have.members([properties[1], properties[2], properties[3], properties[4]], 'Failed to share properties as favorites on first user');
    ...
    chai_1.expect(favorites).to.have.members([properties[1], properties[2], properties[3], properties[4], properties[5], properties[6]], 'Failed to share properties as favorites on fifth user');
    console.log('Properties are shared successfully on first user');
    await userService.toggleFavorite(users[0], properties[2]);
    await userService.toggleFavorite(users[0], properties[6]);
    favorites = (await userService.getFavorites(users[0])).map(item => item.id);
    chai_1.expect(favorites).to.have.members([properties[1], properties[3], properties[4], properties[6]], 'The deleted property reference is not removed on favorites on first user');
    console.log('The deleted property is removed on the favorites of first user');
    await userService.toggleFavorite(users[5], properties[2]);
    await userService.toggleFavorite(users[5], properties[6]);
    favorites = (await userService.getFavorites(users[5])).map(item => item.id);
    chai_1.expect(favorites).to.have.members([properties[1], properties[3], properties[4], properties[5]], 'The deleted property is not removed on the favorites on fifth user');
